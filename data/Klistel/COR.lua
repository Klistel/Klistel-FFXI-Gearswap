...
function user_setup()
    ...
    state.Gun = M{['description']='Gun', 'Anarchy', 'DeathPenalty', 'Fomalhaut', 'Armageddon'}
    state.WeaponSet = M{['description']='Melee', 'Sword', 'Dagger','Tauret', 'Ranged', 'LeadenRangeTP', 'LeadenMeleeTP','Blunt', 'lvl1dagger'}
    ...
    send_command('bind ^numpad8 input /ws "Requiescat" <t>')
    ...
    send_command('bind ^numpad3 input /ws "Viper Bite" <t>')
...
function job_post_precast(spell, action, spellMap, eventArgs)
    if spell.type == 'CorsairRoll' or spell.english == "Double-Up" then
        ...
        if state.WeaponLock.value == false and spell.english ~= "Double-Up" then
            send_command('input //du blinking all all on')
            equip(sets.precast.CorsairRoll.Weapons)
        else
            send_command('input //du blinking all all on')
...
function job_aftercast(spell, action, spellMap, eventArgs)
    if (spell.type == 'CorsairRoll' or spell.english == "Double-Up") and not spell.interrupted then
        ...
        send_command('input //du blinking all all off')
    end
...
function check_gear()
    ...
    if no_swap_gear:contains(player.equipment.waist) then
        disable("waist")
    elseif state.th_gear_is_locked == false then
        enable("waist")
    end
end
